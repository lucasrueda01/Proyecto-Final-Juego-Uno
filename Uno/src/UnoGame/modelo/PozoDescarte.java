		...
		if ((carta.getColor() == null) && (!ignorar)) { // Si la carta es un comodin
			...
			case CAMBIA_COLOR_ROBA_CUATRO:
				if ((!ignorar) && (carta.getColor().equals(this.color)))  {
					...
					if (carta.getTipo().equals(Tipos.CAMBIA_COLOR_ROBA_CUATRO)) {
						b = true;
					}
				}
				break;
			case ROBA_DOS:
				if (!ignorar) {
					if ((carta.getColor().equals(this.color)) || (carta.getTipo().equals(Tipos.ROBA_DOS)))
						b = true;
				} else {
					if (carta.getTipo().equals(Tipos.ROBA_DOS))
						b = true;
				}
				break;
			default:
